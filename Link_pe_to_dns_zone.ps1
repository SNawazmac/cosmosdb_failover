...
[Parameter(Mandatory=$true)][string]$cosmosdb_private_dns_zone_name,                    #Enter the name of the existing Private DNS Zone
[Parameter(Mandatory=$true)][string]$cosmosdb_account_private_dns_zone_group_name,       #Enter the name of the existing Private DNS Zone group
...
$Zone = Get-AzPrivateDnsZone -ResourceGroupName $private_dns_zone_resource_group_name -Name $cosmosdb_private_dns_zone_name
$PrivateDnsZoneId = $zone.ResourceId

#The below commands creates config for private DNS zone integration
$config = New-AzPrivateDnsZoneConfig -Name $cosmosdb_private_dns_zone_name -PrivateDnsZoneId $PrivateDnsZoneId
